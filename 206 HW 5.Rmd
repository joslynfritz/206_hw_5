...
med_salary_male <- ggplot(data = emp_med_salary, aes(x = field, y = male, fill = status)) +   
  geom_bar(stat = "identity", position = "dodge")

ggplot(warp.m, aes(x = Wool, y = Breaks, group = Tension)) +
geom_col(aes(fill = Tension),
position = "dodge") +
scale_fill_manual(values = c("gray70","gray40","gray20")) +
theme(panel.grid.major = element_blank(),
panel.grid.minor = element_blank(),
panel.background = element_blank(),
axis.line = element_line(colour = "black")) +
xlab("Wool Type") +
ylab("Number of Breaks\nRecorded During Spinning") +
ggtitle("Wool Strength by\nType and Tension") +
...
med_salary_female <- ggplot(data = med_salary, aes(x = field, y = female, fill = status)) +   geom_bar(stat = "identity", position = "dodge")

med_salary_female


test <- ggplot(data = emp_med_salary, aes(x = field, fill = male + female)) +   geom_bar(stat = "identity", position = "dodge")
...
faculty_salary_new <- faculty_salary %>% 
  mutate(
    Rank = case_when(
      Faculty_Rank == "Prof" ~ 1,
      Faculty_Rank == "AssocProf" ~ 2,
      Faculty_Rank == "AsstProf" ~ 3
    )
  ) %>% 
